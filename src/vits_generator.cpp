 ...
 * Module:      vits
 * Purpose:     Builds synthesis plans from VITS catalog entries for future line rendering.
 *
...
int PrimitiveRampSamples(const VitsPlannedPrimitive& primitive, double sample_rate_hz) {
  return RiseTimeToRampSamples(primitive.definition.rise_time_us * 1.0e-6, sample_rate_hz);
}

double CarrierPhase(double frequency_hz,
...
double SinSquaredEnvelope(int relative_index, int width_samples) {
  if (relative_index < 0 || relative_index >= width_samples || width_samples <= 1) {
    return 0.0;
  }
...
bool ApplyPrimitive(const VitsPlannedPrimitive& primitive,
  ...
  const int unclamped_start = TimeUsToSampleIndex(primitive.definition.start_us, sample_rate_hz);
  const int unclamped_end = TimeUsToSampleIndex(primitive.definition.end_us, sample_rate_hz);
  ...
  for (int sample_index = start; sample_index < end; ++sample_index) {
    ...
    const double baseline_mv = SampleFixedToMillivolts((*samples)[static_cast<std::size_t>(sample_index)]);
    ...
    switch (primitive.definition.primitive_type) {
      ...
      case VitsPrimitiveType::kBurst: {
        const double envelope = ShapedGateEnvelope(relative_index, width_samples, ramp_samples);
        const int oscillator_sample_index =
            primitive.definition.subcarrier_lock_multiple > 0.0 ? sample_index : (sample_index - start);
        const double phase = CarrierPhase(primitive.resolved_frequency_hz,
                                          primitive.phase_radians,
                                          oscillator_sample_index,
                                          sample_rate_hz);
        target_mv = primitive.dc_offset_mv +
                    (primitive.level_or_amplitude_mv * envelope * std::sin(phase));
        break;
      }
      case VitsPrimitiveType::kSinSquaredPulse: {
        const double envelope = SinSquaredEnvelope(relative_index, width_samples);
        target_mv = primitive.dc_offset_mv + (primitive.level_or_amplitude_mv * envelope);
        break;
      }
      ...
      case VitsPrimitiveType::kStaircase: {
        ...
        const int clamped_relative = std::min(relative_index, width_samples - 1);
        const int step_index = std::min(
            steps - 1,
            static_cast<int>((static_cast<long long>(clamped_relative) * steps) /
                             std::max(1, width_samples)));
        const double step_level = primitive.level_or_amplitude_mv *
                                  (static_cast<double>(step_index + 1) /
    ...
    if (primitive.definition.combine_mode == VitsCombineMode::kReplace) {
      const double gate = ShapedGateEnvelope(relative_index, width_samples, ramp_samples);
      const double output_mv = baseline_mv + ((target_mv - baseline_mv) * gate);
      (*samples)[static_cast<std::size_t>(sample_index)] = MillivoltsToSampleFixed(output_mv);
      continue;
    }

    // Add-mode colour bars/staircases carry explicit rise_time_us in the catalog;
    // apply the same gate envelope so they ramp in/out instead of hard stepping.
    ...
    (*samples)[static_cast<std::size_t>(sample_index)] += MillivoltsToSampleFixed(add_mv);
  }

  return true;
}

const VitsPlannedPrimitive* FindPrimitive(const VitsSynthesisPlan& plan, const std::string& id) {
...
const VitsCompositeDefinition* FindComposite(const VitsSynthesisPlan& plan, const std::string& id) {
...
void ApplySerialCrossfadeTransitionPolicies(VitsSynthesisPlan* plan) {
  ...
  for (const VitsCompositeDefinition& composite : plan->composites) {
    if (composite.mode != VitsCompositeMode::kSerial || composite.children.size() < 2) {
      continue;
    }
    ...
    for (std::size_t i = 0; i + 1 < composite.children.size(); ++i) {
      ...
      if (current_it == primitive_index.end() || next_it == primitive_index.end()) {
        continue;
      }
      ...
      if (current.definition.transition_out_policy != VitsTransitionOutPolicy::kCrossfade ||
          current.definition.transition_out_duration_us <= 0.0) {
        continue;
      }

      if (current.definition.signal_component != next.definition.signal_component ||
          current.definition.combine_mode != next.definition.combine_mode) {
        continue;
      }

      if (current.definition.continuity_group.empty() && composite.continuity_group.empty()) {
        continue;
      }
      ...
      next.definition.start_us =
          std::max(current.definition.start_us, next.definition.start_us - crossfade_us);
    }
  }
}
...
void ApplyGateEnvelopeOverlapCorrections(VitsSynthesisPlan* plan) {
  ...
  // For each primitive with a gate envelope in replace mode, find and create overlap
  // with the next primitive on the same signal component to prevent "drop to zero" artifacts.
  for (std::size_t i = 0; i < plan->primitives.size(); ++i) {
    ...
    // Prefer explicit continuity chains when the catalog marks a construction sequence.
    ...
    for (std::size_t j = 0; j < plan->primitives.size(); ++j) {
      ...
      // Continuity joins are component-local; Y and C chains must not cross-couple.
      if (candidate.definition.signal_component != current.definition.signal_component) {
        continue;
      }

      if (!current.definition.continuity_group.empty() &&
          current.definition.continuity_group == candidate.definition.continuity_group &&
          j > i) {
        best_index = j;
      ...
      const double time_gap = candidate.definition.start_us - current.definition.end_us;
    ...
    // so the handoff is synthesized across the connection instead of landing on it.
    if (best_index < plan->primitives.size()) {
      VitsPlannedPrimitive& next = plan->primitives[best_index];
      
        // Gate-envelope overlap correction is only valid for replace-mode handoffs.
        // Add-mode burst chains use explicit transition policies and must not be
        // stretched by this pass.
        if (current.definition.combine_mode != VitsCombineMode::kReplace ||
          next.definition.combine_mode != VitsCombineMode::kReplace) {
        continue;
      }

      const double join_padding_us = current.definition.rise_time_us + next.definition.rise_time_us;
      current.definition.end_us += join_padding_us;
      next.definition.start_us = std::max(0.0, next.definition.start_us - join_padding_us);
    }
  }
}

bool ApplyNode(const VitsSynthesisPlan& plan,
               const std::string& id,
               double sample_rate_hz,
...
bool ApplyComposite(const VitsSynthesisPlan& plan,
  ...
  if (composite.mode == VitsCompositeMode::kSerial) {
    for (const std::string& child_id : composite.children) {
      if (!ApplyNode(plan, child_id, sample_rate_hz, y_samples, c_samples, error)) {
        return false;
      }
    }
    return true;
...
bool ApplyNode(const VitsSynthesisPlan& plan,
  ...
  if (const VitsPlannedPrimitive* primitive = FindPrimitive(plan, id)) {
    std::vector<SampleFixed>* target = primitive->definition.signal_component == VitsSignalComponent::kY
                                           ? y_samples
                                           : c_samples;
  ...
  if (const VitsCompositeDefinition* composite = FindComposite(plan, id)) {
    return ApplyComposite(plan, *composite, sample_rate_hz, y_samples, c_samples, error);
  }
...
bool VitsGenerator::BuildSynthesisPlan(const VitsDefinition& definition,
  ...
  for (const VitsPrimitiveDefinition& primitive : definition.primitives) {
    ...
    planned.dc_offset_mv = ToMillivolts(primitive.dc_offset, definition.levels_unit);
    if (primitive.frequency_mhz > 0.0) {
      ...
      planned.resolved_frequency_hz = primitive.subcarrier_lock_multiple * subcarrier_hz;
    }
...
bool VitsGenerator::RenderLine(const VitsSynthesisPlan& plan,
  ...
  if (sample_rate_hz <= 0.0 || sample_count <= 0) {
    if (error != nullptr) {
      *error = "VITS line rendering requires positive sample_rate_hz and sample_count.";
    }
    return false;
  }
  ...
  for (const std::string& node_id : plan.render_order) {
    if (!ApplyNode(plan,
                   node_id,
                   sample_rate_hz,
                   &out_line->y_samples_mv,
                   &out_line->c_samples_mv,
                   error)) {
      return false;
    }
  }
