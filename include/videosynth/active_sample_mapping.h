 ...
 * Purpose:     Defines deterministic active-sample to source-pixel mapping
 * helpers.
 *
...
inline int MapActiveSampleToSourcePixel(int sample_index,
  ...
  const int mapped = source_active_x + ((sample_index * source_active_width) /
                                        active_sample_count);
...
inline int MappedSampleCountForSourcePixel(int source_pixel_index,
                                           ...
                                           int active_sample_count) {
  if (active_sample_count <= 0 || source_active_width <= 0 ||
      source_pixel_index < 0 || source_pixel_index >= source_active_width) {
    return 0;
  }

  const int start =
      (source_pixel_index * active_sample_count + source_active_width - 1) /
      source_active_width;
  const int end = (((source_pixel_index + 1) * active_sample_count +
                    source_active_width - 1) /
                   source_active_width) -
                  1;
