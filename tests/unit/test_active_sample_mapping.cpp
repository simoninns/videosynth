 ...
 * Purpose:     Validates deterministic active-sample mapping coverage and
 * endpoint guarantees.
 *
...
TEST(ActiveSampleMappingTest,
     EndpointsMapToFirstAndLastSourcePixelForPalAndNtsc) {
  for (const int active_samples : {922, 745}) {
    for (const int width : {704, 720}) {
      ...
      EXPECT_EQ(MapActiveSampleToSourcePixel(active_samples - 1, active_samples,
                                             width, x0),
                x0 + width - 1);
    }
  }
  ...
  for (const int active_samples : {922, 745}) {
    for (const int width : {704, 720}) {
      ...
      for (int s = 0; s < active_samples; ++s) {
        const int pixel_x =
            MapActiveSampleToSourcePixel(s, active_samples, width, x0);
      ...
      const int min_count =
          *std::min_element(per_pixel_counts.begin(), per_pixel_counts.end());
      EXPECT_GE(min_count, 1);
      EXPECT_EQ(
          std::accumulate(per_pixel_counts.begin(), per_pixel_counts.end(), 0),
          active_samples);
    }
  }
}
...
TEST(ActiveSampleMappingTest,
     SourcePixelSampleSpanDistributionIsDeterministic) {
  for (const int active_samples : {922, 745}) {
    for (const int width : {704, 720}) {
      std::vector<int> per_pixel_counts(static_cast<std::size_t>(width), 0);
      ...
      const int min_count =
          *std::min_element(per_pixel_counts.begin(), per_pixel_counts.end());
      const int max_count =
          *std::max_element(per_pixel_counts.begin(), per_pixel_counts.end());
