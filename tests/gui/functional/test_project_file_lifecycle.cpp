...
std::vector<std::filesystem::path> ExampleProjectFiles() {
  return test_support::AllProjectFixtures();
}
...
// Mirrors the GUI's Open -> edit-free -> Save As flow over every project
// fixture: the re-saved file must parse to the same project and pass the
