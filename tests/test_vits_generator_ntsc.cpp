...
VitsRenderedLine RenderCatalogType(const std::string& vits_type) {
  ...
  std::string error;
  EXPECT_TRUE(provider.TryGetDefinition(Standard::kNtsc, vits_type, &definition, &error)) << error;
...
double IreToMv(double ire) {
  return ire * kMillivoltsPerIre;
}
...
TEST(VitsGeneratorNtscTest, Ntc7CompositeWhiteReference) {
  ...
  const double white = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 14.0, 28.0);
...
TEST(VitsGeneratorNtscTest, Ntc7CompositePulse2tPeak) {
  ...
  const double peak = SampleAtUs(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 34.0);
...
TEST(VitsGeneratorNtscTest, Ntc7CompositeModulatedPulseY) {
  ...
  const double y_mean = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 35.4, 38.6);
...
TEST(VitsGeneratorNtscTest, Ntc7CompositeModulatedPulseC) {
  ...
  const double c_abs_mean = WindowAbsMean(
      rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 35.4, 38.6);
...
TEST(VitsGeneratorNtscTest, Ntc7CompositeChrominanceReference) {
  ...
  const double c_abs_mean = WindowAbsMean(
      rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 44.0, 58.0);
  EXPECT_GT(c_abs_mean, 50.0);
}
...
TEST(VitsGeneratorNtscTest, Ntc7CompositeStaircaseStepLevels) {
  ...
  const double level_start = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 46.0, 48.0);
  const double level_end = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 58.0, 60.0);
...
// Spec: grey_background (50 IRE, 12-62us), grey_reference_boost (+50 IRE, 12-16us),
...
TEST(VitsGeneratorNtscTest, Ntc7CombinationGreyBackground) {
  ...
  const double grey = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 12.0, 62.0);
...
TEST(VitsGeneratorNtscTest, Ntc7CombinationGreyReferenceBoost) {
  ...
  const double boosted = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 13.0, 15.0);
...
TEST(VitsGeneratorNtscTest, Ntc7CombinationMultiburstPackets) {
  ...
  const double chroma_18_23 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 19.0, 22.0);
  const double chroma_24_27 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 24.5, 26.5);
  const double chroma_28_31 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 28.5, 30.5);
  const double chroma_32_35 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 32.5, 34.5);
  const double chroma_36_39 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 36.5, 38.5);
  const double chroma_40_43 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 40.5, 42.5);
...
TEST(VitsGeneratorNtscTest, Ntc7CombinationChromaStaircase) {
  ...
  const double zone1 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 46.5, 49.5);
  const double zone2 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 50.5, 53.5);
  const double zone3 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 55.0, 59.0);
...
// Spec: grey_pedestal (40 IRE, 9.2-62us), white_reference_boost (+60 IRE, 9.2-15.7us),
...
TEST(VitsGeneratorNtscTest, FccMultiburstGreyPedestal) {
  ...
  const double pedestal = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 10.0, 60.0);
...
TEST(VitsGeneratorNtscTest, FccMultiburstWhiteReferenceBoost) {
  ...
  const double boosted = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 10.0, 14.0);
  EXPECT_NEAR(boosted, IreToMv(100.0), 15.0);
}
...
TEST(VitsGeneratorNtscTest, FccMultiburstBurstTrain) {
  ...
  const double amp_0_5 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 20.0, 25.0);
  EXPECT_GT(amp_0_5, kExpectedAmplitude * 0.5);

  // 1.25 MHz (28.2-34.2 us)
  const double amp_1_25 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 30.0, 33.0);
  EXPECT_GT(amp_1_25, kExpectedAmplitude * 0.5);

  // 2.0 MHz (35.2-40.2 us)
  const double amp_2_0 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 36.0, 39.5);
  EXPECT_GT(amp_2_0, kExpectedAmplitude * 0.5);

  // 3.0 MHz (41.2-46.2 us)
  const double amp_3_0 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 42.0, 45.0);
  EXPECT_GT(amp_3_0, kExpectedAmplitude * 0.5);

  // 3.58 MHz (subcarrier locked, 47.2-52.2 us)
  const double amp_3_58 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 48.0, 51.0);
  EXPECT_GT(amp_3_58, kExpectedAmplitude * 0.5);

  // 4.1 MHz (53.2-58.2 us)
  const double amp_4_1 = WindowAbsMean(rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 54.0, 57.0);
...
//       modulated_pulse (Y:50 IRE 37.9-41.1us, C:50 IRE 37.9-41.1us phase 180°),
//       white_reference (100 IRE, 43.9-62us)
...
TEST(VitsGeneratorNtscTest, FccCompositeChromaReferenceZone) {
  ...
  const double amplitude = CorrelateAmplitude(
      rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 12.0, 26.0, ntsc_subcarrier_hz, kPi);
...
TEST(VitsGeneratorNtscTest, FccCompositeStaircase) {
  ...
  EXPECT_NEAR(SampleAtUs(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 15.0), IreToMv(16.0), 10.0);
  EXPECT_NEAR(SampleAtUs(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 18.0), IreToMv(32.0), 10.0);
  EXPECT_NEAR(SampleAtUs(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 21.0), IreToMv(48.0), 10.0);
  EXPECT_NEAR(SampleAtUs(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 24.0), IreToMv(64.0), 10.0);
  EXPECT_NEAR(SampleAtUs(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 27.0), IreToMv(80.0), 10.0);
}
...
TEST(VitsGeneratorNtscTest, FccCompositePulse2t) {
  ...
  EXPECT_GT(SampleAtUs(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 35.5), 680.0);
}
...
TEST(VitsGeneratorNtscTest, FccCompositeModulatedPulse) {
  ...
  const double y_mean = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 37.9, 41.1);
  EXPECT_GT(std::abs(y_mean), 100.0);

  // Modulated C should be present in the 37.9-41.1 us window
  const double c_abs_mean = WindowAbsMean(
      rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 37.9, 41.1);
  EXPECT_GT(c_abs_mean, 50.0);
}
...
TEST(VitsGeneratorNtscTest, FccCompositeWhiteReference) {
  ...
  // White reference at 100 IRE = 714.3 mV
  const double white = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 46.0, 60.0);
...
TEST(VitsGeneratorNtscTest, VirsFirstZoneLevel) {
  ...
  const double first_zone = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 12.0, 34.0);
...
TEST(VitsGeneratorNtscTest, VirsChromaReference) {
  ...
  const double ntsc_subcarrier_hz = 3579545.0;
  const double amplitude = CorrelateAmplitude(
      rendered.c_samples_mv, ntsc.sample_rate_4fsc_hz, 12.0, 32.0, ntsc_subcarrier_hz, kPi);
...
TEST(VitsGeneratorNtscTest, VirsSecondZoneLevel) {
  ...
  const double second_zone = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 37.0, 47.0);
...
TEST(VitsGeneratorNtscTest, VirsPostBlankLevel) {
  ...
  const double post_blank = WindowMean(rendered.y_samples_mv, ntsc.sample_rate_4fsc_hz, 50.0, 60.0);
