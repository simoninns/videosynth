 ...
 * Purpose:     Integration tests for VITS line injection in the generation
 * pipeline.
 *
...
Project CreateProjectWithVits(Standard standard, const std::string& vits_type,
                              int target_line) {
  Project project;
...
int GetSampleIndexForLine(
    [[maybe_unused]] const std::vector<SampleFixed>& samples,
    const TimingConstants& timing, int line_number, double time_us) {
  const int samples_per_line = timing.samples_per_line_4fsc;
  const int line_offset = (line_number - 1) * samples_per_line;
  const int time_offset = static_cast<int>(
      std::lround((time_us * 1e-6) * timing.sample_rate_4fsc_hz));
...
double MeanLevelOnLine(const std::vector<SampleFixed>& samples,
  ...
  const int start_idx =
      GetSampleIndexForLine(samples, timing, line_number, start_us);
  const int end_idx =
      GetSampleIndexForLine(samples, timing, line_number, end_us);

  if (start_idx >= static_cast<int>(samples.size()) ||
      end_idx > static_cast<int>(samples.size())) {
    return 0.0;
  }

  double sum = 0.0;
  int count = 0;
  for (int i = start_idx; i < end_idx && i < static_cast<int>(samples.size());
       ++i) {
...
TEST(LineInjectionVitsIntegrationTest, PalVitsTypesAllRenderable) {
  const std::vector<std::pair<std::string, int>> pal_vits_types = {
      {"vits17", 17}, {"itu-multiburst", 18}, {"uk-national", 19},
      {"vits20", 20}, {"itu-composite", 330}, {"itu-combination", 331},
  };
...
TEST(LineInjectionVitsIntegrationTest, MultipleVitsLinesInSameFrame) {
  ...
  // Line 18: itu-multiburst has grey pedestal at 350 mV (measure in stable
  // region after reference bars)
...
TEST(LineInjectionVitsIntegrationTest, VitsCoexistsWithActiveVideo) {
  ...
  const double mean_active =
      MeanLevelOnLine(y_samples, timing, 100, 14.0, 20.0);

  // Active video should have color bar content (not 700 mV white)
  // Color bars have various levels, so we just check it's not the VITS white
  // reference This is a weak check but verifies different content
